...
export function LoginForm({ onLogin }: LoginFormProps): ReactNode {
  ...
  return (
    <form className="login-card" method="post" action="/api/v1/auth/login" onSubmit={handleSubmit}>
      ...
      <p className={`panel-meta login-message${message.variant ? ` is-${message.variant}` : ""}`} role="status">
        {message.text}
      </p>
