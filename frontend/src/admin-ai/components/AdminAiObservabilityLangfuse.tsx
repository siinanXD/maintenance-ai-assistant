...
export function AdminAiObservabilityLangfuse({
  ...
  return (
    <section className="panel ai-observability-langfuse">
      ...
      <div className="ai-observability-langfuse-status">
        <article className="metric-card">
          ...
          <strong>{ready ? "bereit" : enabled ? "konfiguration prüfen" : "deaktiviert"}</strong>
        </article>
        <article className="metric-card">
          <span>Host</span>
          <strong>
        ...
        <article className="metric-card">
          <span>Schlüssel</span>
          <strong>{configured ? "gesetzt" : "fehlt"}</strong>
        </article>
      </div>
      <div className="dashboard-grid dashboard-grid-4 mt-4">
        {LANGFUSE_KPIS.map(([key, label]) => (
          <article className="metric-card" key={key}>
            <span>{label}</span>
            <strong>{langfuseMetricValue(metricState, key)}</strong>
          </article>
        ))}
      </div>
      {!ready ? (
        <p className="panel-meta mt-4">
