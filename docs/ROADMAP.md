...
| Priority | Area | Notes |
| --- | --- | --- |
| Medium | Long service modules | `retrieval_evaluation_service.py` (1,460 lines), `ai_service.py` (1,100), `retrieval_telemetry_service.py` (1,000) and `knowledge_gap_service.py` (990) each hold several concerns. Split the way `task_service` and `ai_observability_*` were split: one module per concern, cross-module names public, no re-exports |
